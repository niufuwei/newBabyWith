...
{ 
    ...
    for(i = 0; i < MAX_PPPP_CHANNEL_NUM; i++)
    {
        if(m_PPPPChannel[i].bValid == 0)
        {
            ...
            m_PPPPChannel[i].pPPPPChannel->m_SensorAlarmDelegate = pCameraViewController; //这里要设置 报警的代理！！！！
...
int CPPPPChannelManagement::SetSensorAlarmDelegate(char *szDID, id delegate)
{
    [m_Lock lock];
    ...
    for(i = 0; i < MAX_PPPP_CHANNEL_NUM; i++)
    {
        if(m_PPPPChannel[i].bValid == 1 && strcmp(m_PPPPChannel[i].szDID, szDID) == 0)
        {
            m_PPPPChannel[i].pPPPPChannel->SetSensorAlarmDelegate(delegate);
            [m_Lock unlock];
            return 1;
