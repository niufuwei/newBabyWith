...
CPPPPChannel::CPPPPChannel(CCircleBuf *pVideoBuf, CCircleBuf *pPlaybackVideoBuf, const char *DID, const char *user, const char *pwd)
{
    ...
    m_SensorAlarmLock = [[NSCondition alloc] init];
...
{   NSLog(@"2");
    ...
    [m_SensorAlarmLock release];
    m_SensorAlarmLock  = nil;
...
void CPPPPChannel::CommandRecvProcess()
{
    ...
    while(m_bCommandRecvThreadRuning)
    {
       ...
       // NSString *str = [NSString stringWithCString:pbuf encoding:NSUTF8StringEncoding];
        NSLog(@"11111111111111111111111111111%@",str);
        NSArray * arr = [str componentsSeparatedByString:@";"];
        NSLog(@"22222222222222222222222222222%@",arr);
        SAFE_DELETE(pbuf);
...
//                ProcessNetworkParams(networkParams);
        ...
        case CGI_IEGET_RECORD:
        {
            ...
            break;
        case CGI_SENSOR_ALARM:
        {
            STRU_SENSOR_ALARM_INFO sensoralarmInfo;
            
            memset(&sensoralarmInfo, 0, sizeof(sensoralarmInfo));
            
            memcpy(&sensoralarmInfo, pbuf, sizeof(sensoralarmInfo));
            
            NSLog(@"pbuf  Enable %d, SensorAction  %d,SensorID[0] %d,SensorID[1] %d,SensorID[2] %d,LinkPreset %d, SensorName-%s-",sensoralarmInfo.cmd, sensoralarmInfo.SensorAction, sensoralarmInfo.SensorID[0],sensoralarmInfo.SensorID[1],sensoralarmInfo.SensorID[2],sensoralarmInfo.LinkPreset,sensoralarmInfo.SensorName);
            
            ProcessSensorAlarm(sensoralarmInfo);
...
void CPPPPChannel::ProcessSensorAlarm(STRU_SENSOR_ALARM_INFO alarmInfo){
    
    NSLog(@"25");
    [m_SensorAlarmLock lock];
    NSLog(@"cmd.motioninfo is %d",alarmInfo.cmd);
    
    
    if (m_SensorAlarmDelegate != nil)
    {
    
    if ([m_SensorAlarmDelegate respondsToSelector:@selector(PPPPSensorAlarm:andSensorInfo:)])
    {
        [m_SensorAlarmDelegate PPPPSensorAlarm:[NSString stringWithUTF8String:szDID] andSensorInfo:alarmInfo];
    }
        
    }
    [m_SensorAlarmLock unlock];
...
void CPPPPChannel::AlarmProcess()
{
    ...
    {
        CMD_CHANNEL_HEAD head;
        memset(&head, 0, sizeof(head));
        INT32 res = PPPP_IndeedRead(P2P_ALARM, (CHAR*)&head, sizeof(head));
        if (res < 0) {
            NSLog(@"PPPP Read Head Error : %d",res);
            return;
        }
        
        NSLog(@"PPPP ALARM HEAD Start Code %d Cmd %d Len %d",head.startcode,head.cmd,head.len);
        
        if (head.startcode != CMD_START_CODE) {
            NSLog(@"PPPP Head StartCode != CMD_START_CODE");
            return;
        }
        if (head.len < 0) {
            NSLog(@"PPPP Len < 0");
        ...
        char *pbuf = new char[head.len];
        if (pbuf == NULL) {
            NSLog(@"PPPP Alarm Pbuf == NULL");
        }
        if (head.len == 4) {
            int alarmInfo;
            memset(&alarmInfo, 0, sizeof(alarmInfo));
            res = PPPP_IndeedRead(P2P_ALARM, (CHAR*)&alarmInfo, head.len);
            if (res < 0) {
                NSLog(@"PPPP Read Data Error : %d",res);
                return;
            }
            
            NSLog(@"PPPP Alarm Process %x Alarm Info %d",head.cmd, alarmInfo);
            STRU_SENSOR_ALARM_INFO info;
            memset(&info, 0, sizeof(info));
            //            if (alarmInfo == 1) {
            info.cmd = ALARM_MOTION_INFO;
            ProcessCommand(head.cmd, (char*)&alarmInfo, head.len);
            //            }
        }else{
            STRU_SENSOR_ALARM_INFO info;
            memset(&info, 0, sizeof(info));
            //read data
            res = PPPP_IndeedRead(P2P_ALARM, (CHAR*)&info, head.len);
            if (res < 0) {
                NSLog(@"PPPP Alarm Process %x",head.cmd);
                return;
            }
            NSLog(@"SensorAlarmProcessRecv  %x  Enable %d,SensorID[0] %d,SensorID[1] %d,SensorID[2] %d, LinkPreset %d, SensorName-%s-len%d",head.cmd, info.cmd, info.SensorID[0],info.SensorID[1],info.SensorID[2],info.LinkPreset,info.SensorName,head.len);
            ProcessCommand(head.cmd, (char*)&info, head.len);
        }
    }
}
...
void CPPPPChannel::SetSensorAlarmDelegate(id delegate)
{
    NSLog(@"106");
    [m_SensorAlarmLock lock];
    m_SensorAlarmDelegate = delegate;
    [m_SensorAlarmLock unlock];
}
